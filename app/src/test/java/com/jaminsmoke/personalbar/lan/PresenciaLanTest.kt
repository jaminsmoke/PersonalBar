...
class PresenciaLanTest {
    ...
    @Test
    fun encode_escapa_comillas_en_nombre() {
        val json = PresenciaLan.encode(PresenciaLan.Anuncio(establecimiento = "Bar \"Pepe\"", activo = true))
        val dec = PresenciaLan.decode(json)
        assertEquals("Bar \"Pepe\"", dec?.establecimiento)
    }
}
