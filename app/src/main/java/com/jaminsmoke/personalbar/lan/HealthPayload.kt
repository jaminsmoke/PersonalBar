 ...
 * `establecimiento_id` es el UUID de Identity del establecimiento vinculado
 * (se omite cuando el nodo no tiene cuenta Identity, p. ej. demo/offline).
 */
object HealthPayload {
    fun json(
        ...
        append(establecimiento).append('"')
        append(",\"sala\":\"")
        append(establecimiento).append('"')
        if (establecimientoId != null) {
            append(",\"establecimiento_id\":\"")
            append(establecimientoId).append('"')
        }
        append(",\"version\":\"")
