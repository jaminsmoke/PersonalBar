...
def main() -> int:
    ...
    for source in files:
        if not source.exists():
            print(f"MISSING file itself: {source.relative_to(ROOT)}")
            continue
    ...
    if missing:
        ...
        print(f"{len(missing)} reference(s) missing ({checked} checked)")
        return 1

    print(f"OK: {checked} referencias locales existentes")
    return 0
